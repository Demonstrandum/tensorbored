...
The `//tensorbored/data/server` package can be built with Bazel. It builds
...
```
bazel run -c opt //tensorbored -- --grpc_data_provider localhost:6806
```

You don’t have to pass a `--logdir` if you do this, but you do have to
concurrently run `//tensorbored/data/server` (say, in the background, or in a
...
```
bazel run -c opt --define=link_data_server=true //tensorbored -- \
...
```
bazel run -c opt //tensorbored/data/server/pip_package:install
```
...
5.  Run `cargo raze` to update `third_party/rust/...`. This will add or update a
    target like `//tensorbored/data/server/cargo:rand`.
6.  Manually build the crate with Bazel to ensure that it works:
    `bazel build //tensorbored/data/server/cargo:rand`. If the build fails, you
...
`gs://tensorbored-bench-logs` bucket, whose [bucket README] is online. To run
...
RustBoard server is running (`bazel run -c opt //tensorbored/data/server`),
