...
rsync --existing "$1"/tensorflow/core/framework/*.proto tensorbored/compat/proto/
rsync --existing "$1"/tensorflow/core/protobuf/*.proto tensorbored/compat/proto/
rsync --existing "$1"/tensorflow/core/profiler/*.proto tensorbored/compat/proto/
rsync --existing "$1"/tensorflow/core/util/*.proto tensorbored/compat/proto/
rsync --existing "$1"/tensorflow/python/framework/*.proto tensorbored/compat/proto/
...
rsync --existing "$1"/third_party/xla/xla/tsl/protobuf/*.proto tensorbored/compat/proto/
...
find tensorbored/compat/proto/ -type f  -name '*.proto' -exec perl -pi \
  -e 's|tensorflow/core/framework|tensorbored/compat/proto|g;' \
  -e 's|tensorflow/core/protobuf|tensorbored/compat/proto|g;' \
  -e 's|tensorflow/core/profiler|tensorbored/compat/proto|g;' \
...
(
...

  {
    ...
    printf '%s\n' \
      ...
      '# by tensorbored/compat/proto/update.sh.' \
      ''
    ...
    for f in *.proto; do
      ...
      if grep -q '^import\( public\)* "tensorbored/' "$f"; then
        printf '    deps = [\n'
        grep '^import\( public\)* "tensorbored/' "$f" | sort |
  ...
  if command -v buildifier >/dev/null 2>/dev/null; then
    ...
    printf >&2 'warning: buildifier(1) not found; tensorbored/compat/proto/BUILD may have lint errors\n'
  fi
)

echo "Protos in tensorbored/compat/proto/ updated! You can now add and commit them."
