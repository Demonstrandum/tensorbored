...
def tf_ts_library(srcs = [], strict_checks = True, **kwargs):
    ...
    kwargs.setdefault("deps", []).extend(["@npm//tslib", "//tensorbored/defs:strict_types"])
...
def tf_ng_web_test_suite(name, deps = [], **kwargs):
    ...
    extract_js_module_output(
        ...
        deps = [
            ...
            # properly.
            "//tensorbored/webapp/testing:initialize_testbed",
        ] + deps,
...
def tf_svg_bundle(name, srcs, out):
    native.genrule(
        ...
        srcs = srcs,
        outs = [out],
        cmd = "$(execpath //tensorbored/tools:mat_bundle_icon_svg) $(SRCS) > $@",
        tools = [
            "//tensorbored/tools:mat_bundle_icon_svg",
        ],
    )
...
def tf_ng_module(assets = [], **kwargs):
    ...
    tf_ts_library(
        compiler = "//tensorbored/defs:tsc_wrapped_with_angular",
...
def tf_inline_pngs(name, html_template, images, out):
   ...
   Args:
    ...
    native.genrule(
        ...
        outs = [out],
        cmd = "$(execpath //tensorbored/defs:inline_images) $(SRCS) >'$@'",
        tools = ["//tensorbored/defs:inline_images"],
    )
