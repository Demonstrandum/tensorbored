...
of TensorBored (our TensorBoard fork). It creates:
...
- Text summaries (including this source code!)
...
def make_image_summary(tag: str, image: np.ndarray) -> summary_pb2.Summary:
    ...
    return summary_pb2.Summary(value=[
        summary_pb2.Summary.Value(tag=tag, image=image_proto)
    ])
...
def make_histogram_summary(tag: str, values: np.ndarray) -> summary_pb2.Summary:
    ...
    return summary_pb2.Summary(value=[
        summary_pb2.Summary.Value(tag=tag, histo=hist)
    ])
...
def add_summary(writer: EventFileWriter, summary: summary_pb2.Summary, step: int):
...
def setup_default_profile(logdir: Path, run_ids: list):
    ...
    profile_writer.set_default_profile(
        ...
        group_by={"key": "regex", "regexString": r"(baseline|adam|large|small)"},
    )
...
def write_source_code_to_text(writer: EventFileWriter, step: int):
    """Write this script's source code to the text plugin."""
    source_path = Path(__file__)
    with open(source_path, 'r') as f:
        source_code = f.read()
    
    # Format as markdown code block
    text_content = f"""# TensorBored Demo Training Script

This is the actual Python code that generated the training data you're viewing.
It demonstrates how to use TensorBored's features:

- **Profile Writer API**: Configure dashboards from training scripts
- **Color Sampler**: Generate perceptually uniform colors
- **Multiple Summary Types**: Scalars, histograms, images, text
...
The Profile system lets you save, load, and share dashboard configurations.
No more lost settings when you refresh the page!
...
# Create default dashboard profile
profile_writer.set_default_profile(
    logdir=logdir,
    name="My Dashboard",
    pinned_cards=[
...
# Get 5 evenly-spaced perceptually uniform colors
colors = color_sampler.sample_colors(5)
# => ['#dc8a78', '#a4b93e', '#40c4aa', '#7aa6f5', '#d898d5']

# Auto-assign colors to run IDs
run_colors = color_sampler.colors_for_runs(['run1', 'run2', 'run3'])

# For many runs (>8), use varied mode for maximum distinction
colors = color_sampler.sample_colors_varied(15)

# Pre-built palettes
categorical = color_sampler.palette_categorical(5)  # High contrast
sequential = color_sampler.palette_sequential(5)    # Light to dark
diverging = color_sampler.palette_diverging(5)      # Two-ended
```

### 3. Superimposed Cards
...
- **localStorage**: Profiles persist across browser sessions
- **JSON Export/Import**: Share profiles with teammates
- **Up to 1000 pinned cards**: No more URL length limits!

---

*This demo was generated automatically by TensorBored.*
"""
    
    summary = make_text_summary("training_script/source_code", text_content)
...
def main():
    ...
    for exp_name, config in EXPERIMENTS.items():
        ...
        print(f"  Config: lr={config['lr']}, batch_size={config['batch_size']}, optimizer={config['optimizer']}")
        ...
        for step in range(0, TOTAL_STEPS + 1, LOG_EVERY):
            ...
            eval_loss = generate_loss_curve(step, config, seed + 100) * 1.05  # Eval slightly worse
            ...
            add_summary(writer, make_scalar_summary("loss/train", train_loss), step)
            add_summary(writer, make_scalar_summary("loss/eval", eval_loss), step)
            add_summary(writer, make_scalar_summary("accuracy/train", train_acc), step)
            add_summary(writer, make_scalar_summary("accuracy/eval", eval_acc), step)
            ...
            add_summary(writer, make_scalar_summary("gradients/global_norm", max(0.01, grad_norm)), step)
            
            # Histograms (less frequent)
            if step % 50 == 0:
                for layer in ["conv1", "conv2", "fc1", "fc2"]:
                    weights = generate_weight_histogram(step, layer, seed)
                    add_summary(writer, make_histogram_summary(f"weights/{layer}", weights), step)
                    
                    grads = generate_gradient_histogram(step, layer, seed)
                    add_summary(writer, make_histogram_summary(f"gradients/{layer}", grads), step)
            
            # Images (less frequent)
            if step % 100 == 0:
                sample = generate_sample_image(step, seed)
                add_summary(writer, make_image_summary("samples/generated", sample), step)
                
                attention = generate_attention_map(step, seed)
                add_summary(writer, make_image_summary("attention/layer1", attention), step)
            
            # Progress
            if step % 100 == 0:
                print(f"    Step {step}/{TOTAL_STEPS}: loss={train_loss:.4f}, acc={train_acc:.4f}")
        
        # Write source code to text plugin (first run only)
        if exp_name == "baseline":
            write_source_code_to_text(writer, 0)
