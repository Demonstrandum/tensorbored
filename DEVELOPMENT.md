...
(tf)$ pip install tf-nightly -r tensorbored/pip_package/requirements.txt -r tensorbored/pip_package/requirements_dev.txt
...
```sh
(tf)$ bazel test //tensorbored/... --enable_bzlmod --noenable_workspace
```
...
`tensorbored/pip_package/requirements_bazel_lock.txt`. After changing either
...
(tf)$ uv pip compile --python-version 3.10 --generate-hashes \
    --output-file tensorbored/pip_package/requirements_bazel_lock.txt \
    tensorbored/pip_package/requirements_bazel.in
```
...
yarn run ibazel run tensorbored:dev -- -- --logdir path/to/logs
```
...
docker run \
    -p 6006:6006 \
    -v $PWD/tensorbored:/tensorbored/tensorbored \
    -v $MY_LOG_DIR:/logdir \
    tensorflow/tensorboard \
    ibazel run tensorbored:dev -- --logdir /logdir --bind_all
```
...
Note that it may still be necessary to run tensorbored natively in order to test
...
# port.
bazel run tensorbored/components/vz_sorting/test:test_web_library
...
4.  Rebuild and test TensorBoard to make sure it works:
    ...
    * `bazel test --test_output=errors tensorbored/webapp/... tensorboard/components/... tensorboard/plugins/...`
